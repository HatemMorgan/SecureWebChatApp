...
public class UserRestService {
	...
	public static void main(String[] args) throws Exception {
		...
		BigInteger privateExponent = new BigInteger(
				"6969969314326684788757048218075709801652660258043315402751582315321228427561845787301752744834957930045024395213531245448943986747638235659735422026661369");
		BigInteger privateModulas = new BigInteger(
				"9030878767778967220908457109745315301613459507154559252488690421288771425181993925836332492165958967519196312333408580963015703994605694235068391369987151");
		...
		// String("MyWo3E7QsZvZaqCKwlb6jqce2aelshhnPeBq3u1yBqnOduJblH+XemRMo2r7UP3gUq8L+/Cl0KyEqOAkM5b74w==:zMjzWoxS6ZnsiOS0QlFmklFirsMKqPl0sB8XWQbGn84=").split(":");
		// String aesKey =
		// decrypt(Base64.getDecoder().decode(cipherText[0].getBytes()),prvKey);
		...
		BigInteger signModulus = new BigInteger(
				"5842315304732174395910317252871816783654489460059964118242354313185333377425654783456914407579194031220048901123342403391992648585492887635752802718862863");
		...
		BigInteger signPrivateExponent = new BigInteger(
				"6546126846981795436790447750647187516394117370692755052371433187731062392852715784297063668374613302716035533133840655182550788832601709883970977305158313");
		BigInteger signPrivateModulas = new BigInteger(
				"7099699061191949418983824684895241005873554373427293889606742297720069439826546086505195964788853064491748962384772612077285895966053821553463572415737447");
		...
		String signature2 = new String(
				Base64.getEncoder().encode(performSigning("Hatem", "SHA256withRSA", signPrvKey)));
		System.out.println("Signature = " + signature2);

		generatetSecretEncryptionKey();
	}
	...
	public static SecretKey generatetSecretEncryptionKey() throws Exception {
		...
		System.out.println(bytesToHex(secKey.getEncoded()).length());
		return secKey;
	}
	...
	public static byte[] encrypt(String plainText, PublicKey pubKey) throws InvalidKeyException,
	...
	public static String decrypt(byte[] byteCipherText, PrivateKey privKey) throws NoSuchAlgorithmException,
	...
	static byte[] performSigning(String s, String alg, PrivateKey privateKey) throws Exception {
		...
		System.out.println("Signature Provider and Algorithm used to generate a signature: " + sign.getProvider() + "  "
				+ sign.getAlgorithm());
	...
	static void performVerification(String s, String alg, byte[] signature, PublicKey publicKey) throws Exception {
		...
		System.out.println("Signature Provider and Algorithm used to verify a signature: " + sign.getProvider() + "  "
				+ sign.getAlgorithm());
		 ...
		 */
		System.out.println("The Signature Verfication  result : " + sign.verify(signature));
	}
